@echo off
REM =============================================================================
REM run_string_proc_test.bat — Run EnergyEstimationPass on string_proc_test.c
REM =============================================================================
REM
REM Runs the full pipeline on the string processing test file:
REM   string_proc_test.c → AArch64 bitcode → MIR → EnergyEstimationPass → HTML
REM
REM Outputs in output\:
REM   report_string_proc.html    — Full HTML report (open in browser)
REM
REM Prerequisites:
...
REM ── This script is in llvm_pipeline\ — find project root
set SCRIPT_DIR=%~dp0
for %%I in ("%SCRIPT_DIR%..") do set "PROJECT_ROOT=%%~fI"

set "LLVM_DIR=%PROJECT_ROOT%\llvm"
set "TOOLS_DIR=/home/karan/miniconda3/envs/llvm_env/bin"
set "TEST_FILE=%PROJECT_ROOT%\string_proc_test.c"
set "BASENAME=string_proc_test"
...
echo.
echo =============================================================================
echo   LLVM EnergyEstimationPass — string_proc_test.c
echo =============================================================================
...
wsl.exe -d Ubuntu -- python3 "%VIZ%" "%WSL_LLVM%/output/energy_results.json" --title "LLVM Pass: %BASENAME%.c" --output "%WSL_LLVM%/output/report_string_proc.html" --model "%MODEL%" 2>"%OUT%\viz_stderr.txt"
copy "%LLVM_DIR%\output\report_string_proc.html" "%OUT%\report_string_proc.html" >nul
echo        OK  →  output\report_string_proc.html
...
echo     report_string_proc.html    — Full HTML report
echo.
echo Opening report in browser...
start "" "%OUT%\report_string_proc.html"
goto :end

REM ===========================================================================
REM wslpath subroutine
REM ===========================================================================
:wslpath
set "__PATH=%~2"
